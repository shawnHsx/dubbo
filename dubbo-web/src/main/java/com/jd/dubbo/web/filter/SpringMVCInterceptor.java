package com.jd.dubbo.web.filter;
 ...
 * 处理器拦截器  使用标准的HTTP请求和HTTP响应作为参数
 * web请求拦截器（WebRequestInterceptor） Web请求拦截器使用了Spring封装类型WebRequest和WebResponse
 */
public class SpringMVCInterceptor implements HandlerInterceptor {
    ...
    /**
     * 该方法将在Controller处理之前进行调用，SpringMVC中的Interceptor拦截器是链式的，可以同时存在
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if (cookies!=null && cookies.length>0){
            for (int i = 0; i < cookies.length; i++) {
                ...
                logger.info("获取当前cookies："+cookie.toString());
            }
        }
        return true;
    }
